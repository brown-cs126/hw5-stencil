...
/*
[read_all(fd, buf, n)] reads exactly [n] bytes of data from the file descriptor
[fd] into [buf], then adds a null byte at the end. Prints an error and exits if
there is a problem with a read (for instance, if the file descriptor isn't open
for reading).
 */
...
descriptor [fd]. Prints an error and exits if there is a problem with a write
(for instance, if the file descriptor isn't open for writing).
 */
...
/*
[open_for_reading(filename)] opens the file named [filename] in read-only mode
and returns a file descriptor, printing an error and exiting if there is a
problem (e.g., if the file does not exist).
 */
...
/*
[open_for_writing(filename)] opens the file named [filename] in write-only mode 
(creating it if it doesn't exist) and returns a file descriptor, printing an error 
and exiting if there is a problem (e.g., if the file is in a directory that doesn't 
exist).
 */
